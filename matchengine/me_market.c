...
market_t *market_create(struct market *conf)
{
    ...
    if (conf->stock_prec + conf->money_prec > asset_prec(conf->money))
        return NULL;
    /* delete 2018/02/05 不检查stock手续费的小数位精度
...
static int append_balance_trade_add(order_t *order, const char *asset, mpd_t *change, mpd_t *price, mpd_t *amount)
{
    ...
    json_object_set_new_mpd(detail, "o", order->amount);
    json_object_set_new(detail, "s", json_string(order->source));
...
static int append_balance_trade_fee(order_t *order, const char *asset, mpd_t *change, mpd_t *price, mpd_t *amount, mpd_t *fee_rate)
{
    ...
    mpd_copy_negate(real_change, change, &mpd_ctx);
    int ret = append_user_balance_history(order->update_time, order->user_id, asset, "trade", real_change, detail_str);
    /* add 2018/02/07 手续费记入EF账户 */
    if (ret == 0)
    {
        ret = append_user_balance_history(order->update_time, EF_USER_ID, asset, "trade", change, detail_str);
    }
    mpd_del(real_change);
    free(detail_str);
    json_decref(detail);
...
static int execute_limit_ask_order(bool real, market_t *m, order_t *taker)
{
    ...
    /* add 2018/02/05 stock手续费小数位精度对应 */
    int prec_save = asset_prec(m->stock);
    if (prec_save < 0)
        return -1;
    ...
    while ((node = skiplist_next(iter)) != NULL)
    {
        ...
        if (maker->user_id == settings.bot_user_id && taker->user_id != settings.bot_user_id)
        {
            json_t *result = NULL;
            log_info("market_cancel_maker_bot_order id: %" PRIu64 ", orderid:%" PRIu64 ", market: %s", maker->id, taker->id, m->name);
            market_cancel_order(true, &result, m, maker);
            json_decref(result);
            continue;
        }
        if (taker->user_id == settings.bot_user_id && maker->user_id != settings.bot_user_id)
        {
            log_info("market_cancel_taker_bot_order id: %" PRIu64 ", orderid:%" PRIu64 ", market: %s", taker->id, maker->id, m->name);
            mpd_sub(taker->left, taker->left, taker->left, &mpd_ctx);
            break;
        }
        ...
        /* add 2018/02/05 stock手续费小数位精度对应:截掉大于精度的小数位 */
        mpd_rescale(bid_fee, bid_fee, -prec_save, &mpd_ctx);
        ...
        if (mpd_cmp(ask_fee, mpd_zero, &mpd_ctx) > 0)
        {
            ...
            balance_add(EF_USER_ID, BALANCE_TYPE_AVAILABLE, m->money, ask_fee); /* add 2018/02/07 手续费记入EF账户 */
            if (real)
            {
        ...
        if (mpd_cmp(bid_fee, mpd_zero, &mpd_ctx) > 0)
        {
            ...
            balance_add(EF_USER_ID, BALANCE_TYPE_AVAILABLE, m->stock, bid_fee); /* add 2018/02/07 手续费记入EF账户 */
            if (real)
            {
